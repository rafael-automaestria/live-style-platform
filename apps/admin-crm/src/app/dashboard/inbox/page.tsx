...
export default async function InboxPage() {
  const leads = await getInboxData();

  // Convert dates to strings for passing to Client Component safely
  const safeProspects = leads.map((p: any) => ({
    ...p,
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950">
      <header className="h-16 flex items-center justify-between px-8 border-b border-zinc-800 shrink-0 bg-zinc-900/50">
        <div>
          <h1 className="text-2xl font-bold text-white">Caixa de Entrada Omnichannel</h1>
          <p className="text-sm text-zinc-400">WhatsApp, Instagram e TikTok Centralizados</p>
        </div>
      </header>
