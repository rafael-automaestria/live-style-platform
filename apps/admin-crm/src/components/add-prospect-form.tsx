  ...
  return (
    <div className="absolute right-8 top-16 mt-2 w-80 bg-zinc-900 border border-zinc-800 rounded-xl p-4 shadow-2xl z-50">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-zinc-100">New Prospect</h3>
      ...
      <form action={async (formData) => {
        ...
        <Input name="name" placeholder="Prospect Name (e.g., Lia Singer)" required className="bg-zinc-950 border-zinc-800" />
        <Input name="platform" placeholder="Platform (e.g., TikTok)" required className="bg-zinc-950 border-zinc-800" />
        <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white">Save Prospect</Button>
      </form>
    </div>
  );
}